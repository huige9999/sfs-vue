          ...
          <div class="layer-flex">
            <div class="layer-title">
              <div
                class="icon"
                style="
                  background-image: url('http://files.zdesigner.cn/2019/12/28/e7eaec524071ce948069035ac5b91ff7.png');
            ...
            <div class="layer-list">
              ...
              >
                <div
                  class="icon"
                  :style="{
                    'background-image': `url(http://files.zdesigner.cn/2020/02/27/8ab5a6ee40d57e448012b8083b68f496.png)`,
                  }"
                ></div>
                  ...
                  <div class="layer-block">
                    <template v-for="(item, $i) in items">
                      <div
                        class="block-item"
                        :key="$i"
                        @click="$router.push(`/tools/item/${item.id}`)"
                      >
                        <div
                          class="icon"
    ...
    <!-- <div class="activity-layer">
      <div class="max-width">
        <div class="layer-title">
          ...
          <div class="layer-nav">
            <div
              :class="actTab === '展览展会' ? 'nav active' : 'nav'"
              @click="getActivity('展览展会')"
            >
              展览展会
            </div>
            <div
              :class="actTab === '知识分享' ? 'nav active' : 'nav'"
              @click="getActivity('知识分享')"
            >
              知识分享
            </div>
            <div
              :class="actTab === '休闲娱乐' ? 'nav active' : 'nav'"
              @click="getActivity('休闲娱乐')"
            >
              休闲娱乐
            </div>
            <div
              :class="actTab === '生活趣味' ? 'nav active' : 'nav'"
              @click="getActivity('生活趣味')"
            >
              生活趣味
            </div>
          </div>
              ...
              <template v-for="(item, $index) in activityList">
                <div class="the-activity" :key="$index" v-if="item">
                    ...
                    <div class="mid-info">
                      ...
                      >
                        <template v-if="item.maxPrice === item.minPrice">
                          <div class="price" v-if="item.minPrice > 0">
                            {{ item.minPrice }}元
                          </div>
                  ...
                  >
                    ...
                    <div
                      class="
                        swiper-agenda swiper-agenda-prev swiper-button-prev
                      "
                    >
                      <i class="iconfont">&#xe693;</i>
                    </div>
                    <div
                      class="
                        swiper-agenda swiper-agenda-next swiper-button-next
                      "
                    >
                      <i class="iconfont">&#xe600;</i>
                    </div>
        ...
        <div class="dialog-flex">
          <div class="dialog-block">
            ...
            <div class="feedback-content">
              <img
                src="http://files.zdesigner.cn/2020/01/07/833f0a7940b5b202804b20accfb30ab8.png"
              />
              <div class="title">宅设小编为您服务</div>
              <div class="content">
                我们运营小组已经收到了您的需求
                会及时处理，请耐心等待哦，您也可以加入我们素材收集组
              </div>
        ...
        for (let i = 0; i < res.length; i += 1) {
          ...
          if (res[i].agenda) {
            ...
            while (index < JSON.parse(res[i].agenda).length) {
              newArray.push(
                JSON.parse(res[i].agenda).slice(index, (index += 3))
              );
            }
          ...
          skusQuery.find().then((respon) => {
            ...
            apQuery.count().then((count) => {
              ...
              if (ar.length > 0) {
                res[i].maxPrice = ar.sort(function (a, b) {
                  return a - b;
                })[ar.length - 1];
                res[i].minPrice = ar.sort(function (a, b) {
                  return a - b;
                })[0];
              }
              ...
              const jsonData = {
                ...
                toggleStatus: false,
                agendaList: res[i].agenda
                  ? JSON.parse(res[i].agenda)
                  : undefined,
                agendaSwiperList: res[i].agenda
                  ? res[i].agendaSwiperList
                  : undefined,
              };
