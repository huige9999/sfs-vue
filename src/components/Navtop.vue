    ...
    <div class="nav-top">
      <div class="logo" @click="$router.push('/')">
        <div
          class="img"
          style="
            background-image: url(http://files.zdesigner.cn/2020/02/21/726f1d504092234a80c547a94b9d50ea.png);
          "
        ></div>
      </div>
      <div class="max-width">
      ...
      <div class="user" v-else>
        <div
          class="user-head"
          :style="{ backgroundImage: `url(${$store.state.user.imgSrc})` }"
        ></div>
        <div class="hello" @click="toUser">
          欢迎，{{ $store.state.user.nickname }}
        </div>
...
export default {
  ...
  created() {
    if (localStorage.getItem("memberInfo")) {
      if (
        ...
        (!JSON.parse(localStorage.getItem("memberInfo")).mobilePhoneNumber ||
          JSON.parse(localStorage.getItem("memberInfo")).mobilePhoneNumber ===
            "" ||
  ...
  methods: {
    testFunc() {
      ...
      const poiID = pointer.set("Ff82333K");
      const query = this.$Bmob.Query("order_list");
      query.get("LHm2GGGZ").then((res) => {
        res.set("product", poiID);
        res.save();
      });

      // 添加Relation
      // const relation = this.$Bmob.Relation("skus"); // 需要关联的表
      // const relID = relation.add("OsFy666L"); //关联表中需要关联的objectId, 返回一个Relation对象, add方法接受string和array的类型参数
      // const query = this.$Bmob.Query("product");
      // query.get("KOY9222S").then((res) => {
      ...
      // query.set("attrNum", 100);
      // query.set("attrPrice", 68);
      // query.set("attrName", 68);
      // query
      //   .save()
      //   .then((res) => {
      //     console.log(res);
    ...
    testMockSaveUserInfo() {
      const mockUser = {
        objectId: 'OEWwHHHL',
        username: 'oo4igjkJKwfv7GzKgz1xIKHiJ2mU',
        password: 'qq123',
        mobilePhoneNumberVerified: false,
        mobilePhoneNumber: '13632590574',
        isCustomer: true,
        isAdmin: true,
        name: '',
        wechatId: '',
        imgSrc: 'https://wx.qlogo.cn/mmopen/vi_32/Q0j4TwGTfTKZcGYFpvcbiasnjyr6oG9Uh4S4NBr7HYsWUr6RgYFibLh7Vya3YBDO6NudPEBibBuYXXGzTGryXibyOQ/132',
        openid: 'oikoL49EaQ0-bTKmXiW3k7V6zU4g',
        unionid: 'oo4igjkJKwfv7GzKgz1xIKHiJ2mU',
        city: 'Meizhou',
        country: 'China',
        sex: 1,
        province: 'Guangdong',
      ...
      localStorage.setItem('memberInfo', JSON.stringify(mockUser));
      this.$store.dispatch("getMember", mockUser);
    },
    toUser() {
      this.$router.push({
        path: "/user/product",
